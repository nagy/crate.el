...
browsing Rust crates from a local crates.io SQLite database.
...
Set =crate-data-path= to the location of your =crates-io.db= SQLite
database, then use =M-x find-crate= to look up a Rust crate.
